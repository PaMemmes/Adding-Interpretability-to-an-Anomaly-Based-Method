...
def plot_comparison_severity_distribution(dist1, dist2, dist3, save=None):
    ...
    rects2 = ax.bar(
        ...
        width=width,
        align='center')
    rects3 = ax.bar(severity_range + width * 1.5, dist3,
                    width=width, align='center')
    ax.bar_label(rects1)
    ax.bar_label(rects2)
    ax.bar_label(rects3)
    ax.legend((rects1[0], rects2[0], rects3[0]),
              ('Normal', 'Fragmented', 'Fragmented Randomly'))
...
def plot_comparison_packet_alerts(
        packets_sum,
        frag_packets_sum,
        rnd_frag_packets_sum,
        sigs_sum,
        frag_sigs_sum,
        rnd_frag_sigs_sum,
        save=None):
    ...
    bar = ax.bar(np.arange(2) - width / 2,
                 [sigs_sum, packets_sum], width=width)
    bar_fragmented = ax.bar(np.arange(2) + width / 2,
                            [frag_sigs_sum, frag_packets_sum], width=width)
    bar_rnd_fragmented = ax.bar(np.arange(
        2) + width * 1.5, [rnd_frag_sigs_sum, rnd_frag_packets_sum], width=width)

    ax.legend((bar[0], bar_fragmented[0], bar_rnd_fragmented[0]),
              ('Normal', 'Fragmented', 'Fragmented Randomly'))
    ax.bar_label(bar)
...
def plot_comparison_categories(
        categories,
        frag_categories,
        frag_rnd_categories,
        save=None):
    ...
    rects1 = ax.bar(severity_range - width / 2, categories,
                    width=width, align='center')
    rects2 = ax.bar(severity_range + width / 2, frag_categories,
                    width=width, align='center')
    rects3 = ax.bar(severity_range + width * 1.5,
                    frag_rnd_categories, width=width, align='center')
    ax.bar_label(rects1)
    ax.bar_label(rects2)
    ax.bar_label(rects3)
    ax.legend((rects1[0], rects2[0], rects3[0]),
