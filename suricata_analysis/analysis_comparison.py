...
class AllDataHolder():
    ...
    def make_comp_plots(self):
        ...
        plot_comparison_packet_alerts(
            self.original.packets_sum,
            self.frag.packets_sum,
            self.frag_random.packets_sum,
            self.original.sigs_sum,
            self.frag.sigs_sum,
            self.frag_random.sigs_sum,
            save='plots/packet_alerts.pdf')
        plot_comparison_severity_distribution(
            self.original.sev_dist,
            self.frag.sev_dist,
            self.frag_random.sev_dist,
            save='plots/severity_dist_comp.pdf')
        plot_comparison_categories(
            self.original.category_dist,
            self.frag.category_dist,
            self.frag_random.category_dist,
            save='plots/category_dist_comp.pdf')
...
class DataHolder():
    ...
    def process_data(self):
        # Processes json files for packets, alerts, severity of alert and
        # category of alert
        ...
        print(
            f'Total categories {self.kind}: {sum(self.category_dist.values())}')
    ...
    def plot_file(self, file):
        ...
        plot_alerts(nmbr_signatures, file, save=f'plots/' +
                    file + '_' + self.kind + '.pdf')

    def make_plots(self):
        # Plots the indivual plots
        plot_alert_distribution(
            self.sigs_dist, save='plots/alert_dist_' + self.kind + '.pdf')
        plot_severity_distribution(
            self.sev_dist, save='plots/severity_dist_' + self.kind + '.pdf')
        plot_packet_alerts(self.packets_sum, self.sigs_sum,
                           save='plots/packet_alerts_' + self.kind + '.pdf')
        plot_categories(self.category_dist,
                        save='plots/cat_dist_' + self.kind + '.pdf')
