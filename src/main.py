...
def run_xg(trials, frags=True):
    ...
    model = xg_main(train=data.train_sqc, test=data.test_sqc, frags=data.test_frag_sqc, trials=trials, save=name)
...
def run_nn(model_name, epochs, retrain, trials, frags=True):
    ...
    train(model_name=model_name, data=data, frags=data.test_frag_sqc, trials=trials, num_retraining=retrain, epochs=args.epochs, save=model_name + '_' + name)
...
def run_combined(epochs, retrain, trials, frags=True):
    ...
    model = xg_main(train=data.train_sqc, test=data.test_sqc, frags=data.test_frag_sqc, trials=args.trials, save='combined' + name)
