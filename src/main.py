...
if __name__ =='__main__':
    ...
    if args.file == 'xg':
        ...
        if args.frag_data =='Y':
            data.preprocess(kind=None, frags=True, add=gan_data)
            ...
            interpret_tree(model, data.test_sqc, data.df_cols, save='train_w_frags_combined')
        else:
            data.preprocess(kind=None, frags=False, add=gan_data)
            ...
            interpret_tree(model, data.test_sqc, data.df_cols, save='train_wo_frags_combined')
        
    elif args.file == 'wgan' or args.file == 'gan':
        if args.frag_data =='Y':
            data.preprocess(kind='normal', frags=True, add=None)
            train(args.file, data.train_sqc, data.test_sqc, data.test_frag_sqc, args.trials, args.retraining, args.epochs, save='train_w_frags_' + args.file)
        else:
            data.preprocess(kind='normal',frags=False, add=None)
            train(args.file, data.train_sqc, data.test_sqc, data.test_frag_sqc, args.trials, args.retraining, args.epochs, save='train_wo_frags_' + args.file)
