...
def make_local_plots(explainer, shap_values, test_x, preds, test_y, df_cols, name):
    a = 0
    b = 0
    c = 0
    d = 0
...
def make_interpret_plots(explainer, shap_values, test_x, preds, test_y, df_cols, name):
    ...
    shap.summary_plot(shap_values, test_x, feature_names=feature_names, show=False)
    f = plt.gcf()
    f.savefig(name + 'summary.pdf', bbox_inches='tight', dpi=300)
    plt.close()

    shap.summary_plot(shap_values, test_x, feature_names=feature_names, max_display=len(df_cols), show=False)
    f = plt.gcf()
    f.savefig(name + 'summary_all.pdf', bbox_inches='tight', dpi=300)
    plt.close()

    make_local_plots(explainer, shap_values, test_x, preds, test_y, df_cols, name)
...
def interpret_tree(model, data, save):
    ...
    preds = model.predict(data.test_frag_sqc.x)
    ...
    for label, test_sqc in data.seperate_tests.items():
        ...
        make_interpret_plots(explainer, shap_values, df, preds, data.test_sqc.y, df_cols, name)
