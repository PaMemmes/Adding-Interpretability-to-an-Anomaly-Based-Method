...
class DataFrame:
    ...
    def make_frags(self, test_size):
        ...
        _, self.x_test_frags, _, self.y_test_frags = train_test_split(
            df, labels, test_size=test_size, shuffle=False)
    ...
    def preprocess(
        ...
        if frags is True:
            self.df = pd.concat([self.df_frag.iloc[int(
                (1 - test_size) * len(self.df_frag)):], self.df], ignore_index=True)
        if add is not None:
        ...
        self.test_frag_sqc = DataSequence(
            self.x_test_frags,
            self.y_test_frags,
